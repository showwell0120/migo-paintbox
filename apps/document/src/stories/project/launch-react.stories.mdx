...
"scripts": {
    ...
    "deploy.stg": "pnpm build && cd ansible; ansible-playbook -i hosts evergreen-frontend.yml --limit staging && git tag git tag staging-`date '+%d%b%Y%H%M%S'` && git push origin --tags",
