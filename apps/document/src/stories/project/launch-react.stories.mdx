...
```bash
"scripts": {
    "deploy.dev": "pnpm build && cd ansible; ansible-playbook -i hosts evergreen-frontend.yml --limit dev && git tag git tag dev-`date '+%d%b%Y%H%M%S'` && git push origin --tags",
    "deploy.stage": "pnpm build && cd ansible; ansible-playbook -i hosts evergreen-frontend.yml --limit staging && git tag git tag staging-`date '+%d%b%Y%H%M%S'` && git push origin --tags",
    "deploy.prod": "pnpm build && cd ansible; ansible-playbook -i hosts evergreen-frontend.yml --limit production && git tag git tag production-`date '+%d%b%Y%H%M%S'` && git push origin --tags"
},
```
