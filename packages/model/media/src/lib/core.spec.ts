import { Core, EventCallbackParam } from './core';

describe('Core', () => {
  it('Get displayName as same as name', () => {
    const instance = new Core({ name: 'Titanic' });
    expect(instance.displayName).toEqual('Titanic');
  });
  it('Get displayName as <undefinedMark> if name is missing', () => {
    const instance = new Core({});
    expect(instance.displayName).toEqual(Core.undefinedMark);
  });
  it('Get dpcp as licensor/cpCode', () => {
    const instance = new Core({ licensor: 'IDGEN', cpCode: 'IDGEN' });
    expect(instance.dpcp).toEqual('IDGEN/IDGEN');
  });
  it('Get dpcp as <undefinedMark>/<undefinedMark> if both fields are missing', () => {
    const instance = new Core({});
  ...
  it('Should work with on method', () => {
    ...
    expect(instance.eventMap.get('active_change')?.length === 1);
  });
  ...
  it('Should work with off method', () => {
    ...
    expect(instance.eventMap.get('active_change')?.length === 0);
  });
